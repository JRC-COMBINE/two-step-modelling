...
% DATE: 08-Jul-2008
% Jonas (2020). Violin Plots for plotting multiple distributions (distributionPlot.m) 
...
% MATLAB Central File Exchange. Retrieved October 15, 2020.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for iData = 1:nData
    ...
    % only plot if there is some finite data and data points with more than
    % one value
    hf = currentData( : ); % added
    
    if ~isempty( currentData( : ) ) && any( isfinite( currentData( : ) ) ) ...
            && length( unique( hf( ~isnan( hf ) ) ) ) > 1
        ...
        switch floor( opt.histOpt )
            ...
            case 1
                ...
                if opt.histOpt == 1.1
                    ...
                    [ xHist, yHist ] = ksdensity( currentData, x, 'kernel', 'normal', 'width', dx/( 1.5 * opt.divFactor ), 'NumPoints', 400 );
                    ...
                    if opt.divFactor ~= 1
                        ...
                        [ xHist, yHist ] = ksdensity( currentData, 'kernel', 'normal', 'width' ,u/opt.divFactor, 'NumPoints', 400 );
                    end
...
for iData = goodData'
    ...
    switch opt.xyOri
        ...
        case 'normal'
            hh(iData)= patch('Vertices',vertices,'Faces',faces,'Parent',ah,colorOpt{:},'EdgeColor','none','FaceAlpha', .2); % normally 0.2 or 0.3
            
        case 'flipped'
            hh(iData)= patch('Vertices',vertices(:,[2,1]),'Faces',faces,'Parent',ah,colorOpt{:},'EdgeColor','none','FaceAlpha', .2); % normally 0.2 or 0.3
    ...
    switch opt.xyOri
        ...
        case 'normal'
            ...
            if any(opt.showMM==[1,2])
                mh = plot(ah,opt.xValues+xOffset,m,'+','Color', [0.6350    0.0780    0.1840], 'MarkerSize', 12);
            end
            
            if any(opt.showMM==[1,3])
                mdh = plot(ah,opt.xValues+xOffset,md,'s','Color',[0.9290    0.6940    0.1250], 'MarkerSize', 12);
            end
            ...
            if opt.showMM == 6
                mdh(1,:) = plot(ah,squeeze(md(:,1,2:3))',repmat(md(:,1,1)',2,1),'color',[0.9290    0.6940    0.1250],'lineWidth',2);%,'lineStyle','--');
                mdh(2,:) = plot(ah,squeeze(md(:,2,2:3))',repmat(md(:,2,1)',2,1),'color',[0.6350    0.0780    0.1840],'lineWidth',1);%,'lineStyle','--');
                mdh(3,:) = plot(ah,squeeze(md(:,3,2:3))',repmat(md(:,3,1)',2,1),'color',[0.6350    0.0780    0.1840],'lineWidth',1);%,'lineStyle','--');
            end
            
        case 'flipped'
            
            if any(opt.showMM==[1,2])
                mh = plot(ah,m,opt.xValues+xOffset,'+','Color',[0.6350    0.0780    0.1840],'MarkerSize',12);
            end
            
            if any(opt.showMM==[1,3])
                mdh = plot(ah,md,opt.xValues+xOffset,'s','Color',[0.9290    0.6940    0.1250],'MarkerSize',12);
            end
            ...
            if opt.showMM == 6
                mdh(1,:) = plot(ah,repmat(md(:,1,1)',2,1),squeeze(md(:,1,2:3))','color',[0.9290    0.6940    0.1250],'lineWidth',2);%,'lineStyle','--');
                mdh(2,:) = plot(ah,repmat(md(:,2,1)',2,1),squeeze(md(:,2,2:3))','color',[0.6350    0.0780    0.1840],'lineWidth',1);%,'lineStyle','--');
                mdh(3,:) = plot(ah,repmat(md(:,3,1)',2,1),squeeze(md(:,3,2:3))','color',[0.6350    0.0780    0.1840],'lineWidth',1);%,'lineStyle','--');
            end
    end
end
